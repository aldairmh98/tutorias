 ...
 * @author aldai
 */
@ManagedBean(name = "students_data")
public class Students_data {

    /**
     * Creates a new instance of Students_
     */
    ...
    public Students_data() {
        if (utils.SessionUtils.getSession().getAttribute("id") != null) {
            ...
            try {
                ...
                tutorias = DAO.DAOTutorias.getDAO().tutoriasByUser(id);
            } catch (Exception ex) {
    ...
    public void selectTeacher() {
        if (currentUser != null) {
            try {
                DAO.DAOUser.getDAO().tutoAssign(currentUser.getId(), teacher_id);
            } catch (SQLException ex) {
                Logger.getLogger(Students_data.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
